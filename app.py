...
def admin_email_logs():
    logs = db.session.execute(text("SELECT * FROM email_logs ORDER BY sent_at DESC")).fetchall()
...
def manage_announcements():
    query = text("SELECT * FROM announcements ORDER BY created_at DESC")
...
def admin_toggle_announcement(ann_id):
    # This will deactivate a specific announcement
    db.session.execute(text("UPDATE announcements SET is_active = FALSE WHERE id = :ann_id"), {'ann_id': ann_id})
    db.session.commit()
